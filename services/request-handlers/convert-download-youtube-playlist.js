      ...
      videoStream.pipe(fs.createWriteStream(videoFilename)).on('close', async () => {
        if (partsDurationArray.length) {
          for (const part of partsDurationArray) {
            timeLogger.start(`splitting-part-${part}-of-${partsDurationArray.length}`);
            let partStream = await videoSplitter(videoFilename, part.start, part.end - part.start)
            timeLogger.end(`splitting-part-${part}-of-${partsDurationArray.length}`);
            timeLogger.start(`converting-part-${part}-of-${partsDurationArray.length}-to-buffer`);
            let partBuffer = await downloadVideoAndReturnBuffer(null, {}, partStream)
            timeLogger.end(`converting-part-${part}-of-${partsDurationArray.length}-to-buffer`);
            timeLogger.start(`sending-part-${part}-of-${partsDurationArray.length}-to-user`);
            await bot.sendVideo(chatId, partBuffer, {}, {
              filename: videoInfo.videoDetails.title.slice(0,12) + `part_${partNumber}` + ".mp4",
              contentType: 'video/mp4',
            });
            timeLogger.end(`sending-part-${part}-of-${partsDurationArray.length}-to-user`);
            partBuffer = [];
            partNumber++
            sleep(700)
          }
